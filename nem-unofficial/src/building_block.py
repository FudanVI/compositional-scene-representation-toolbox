...
class RNN(nn.Module):
    ...
    def __init__(self, input_size, state_size):
        ...
        self.linear = nn.Linear(input_size + state_size, state_size)
        self.layer_norm = nn.LayerNorm(state_size)
        nn.init.kaiming_normal_(self.linear.weight, mode='fan_in', nonlinearity='sigmoid')
        nn.init.zeros_(self.linear.bias)

    def forward(self, inputs, states):
        logits_states = self.linear(torch.cat([inputs, states], dim=-1))
        states = torch.sigmoid(logits_states)
        outputs = torch.sigmoid(self.layer_norm(logits_states))
