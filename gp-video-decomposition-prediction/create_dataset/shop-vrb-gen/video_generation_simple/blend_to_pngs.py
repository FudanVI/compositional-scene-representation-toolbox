...
def generate_overlook(folder, blender_objects):
    if args.use_gpu == 1:
    ...
    ground.scale[1] = 3
    ground.scale[2] = 0
    delta_theta = (args.max_theta - args.min_theta) / 20
    phi = 0.5 * np.pi * 0.58
    rho = 16.5
    for idx in range(20):
        ...
        # Scene
        render_args.filepath = os.path.join(folder, 'overlook_{}.png'.format(idx))
        bpy.ops.render.render(write_still=True)
        # # Segmentation
...
def generate_images(folder, blender_objects):
    ...
    for idx in range(args.num_views):
        ...
        for i, obj in enumerate(blender_objects):
            utils.set_render(obj, True)
            render_args.filepath = os.path.join(folder, 'object_{}_{}.png'.format(idx, step))
            bpy.ops.render.render(write_still=True)
            generate_mask(os.path.join(folder, 'mask_{}_{}.png'.format(idx, step)), blender_objects)
            utils.set_render(obj, False)
            step += 1
        for i, obj in enumerate(blender_objects):
            utils.set_render(obj, True)
...
def main():
    for idx in range(args.offset, args.offset + args.num_images):
        ...
        # generate_overlook(folder_out, blender_objects)
