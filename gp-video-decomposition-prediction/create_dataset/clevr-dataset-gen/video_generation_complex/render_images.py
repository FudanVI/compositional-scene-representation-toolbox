...
def main(args):
  ...
  if args.save_blendfiles != 1 and not os.path.isdir(args.output_image_dir):
...
def add_random_objects(scene_struct, num_objects, args, camera):
  ...
  for i in range(num_objects):
    ...
    theta = properties[obj_name_out]["orientation"]
...
#     return add_random_objects(scene_struct, num_objects, args, camera)
  ...
  indices = [n[0] for n in sorted(enumerate(objects), key=lambda x: x[1]['pixel_coords'][2], reverse=True)]
...
def check_visibility(blender_objects, min_pixels_per_object, height, width, crop_up, crop_down, crop_left, crop_right):
  ...
  _ = render_shadeless(blender_objects, path=path)
