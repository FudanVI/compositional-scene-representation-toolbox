...
def main():
    ...
    path_in = os.path.join(args.folder_in, 'tetrominoes', 'tetrominoes_train.tfrecords')
    dataset = tetrominoes.dataset(path_in, map_parallel_calls=args.num_parallel)
    next_batch = get_next_batch(dataset, args.num_data)
    path_out = os.path.join(args.folder_out, 'tetris')
